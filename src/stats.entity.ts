...
interface OperatorsStats {
  ...
  type: string,
  overall: {
    wins: number, losses: number, kills: number, deaths: number, timeplayed: number, kd: string, winrate: number
  },
  seasonal: {
    wins: number, losses: number, kills: number, deaths: number, timeplayed: number, kd: string, winrate: number
  }
...
export class Stats {
  ...
  constructor(name, stats, ranked, operator, seasons, op_main, aliases) {
    this._name = name;
    this._stats = stats
    this._ranked = ranked
  ...
  getKillsBySeasons(): number[] {
    return Object.entries(this.seasons).map(([key, value]) => value['AS_kills']);
  }

  getDeathsBySeasons(): number[] {
    return Object.entries(this.seasons).map(([key, value]) => value['AS_deaths']);
  }
}
